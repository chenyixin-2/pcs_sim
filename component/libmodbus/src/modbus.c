...
void _error_print(modbus_t *ctx, const char *context)
{
    if (1 /*ctx->debug*/)
    {
        ...
        if (context != NULL)
        {
            log_dbg("ERROR %s : %s", context, modbus_strerror(errno));
        }
        else
        {
            log_dbg("ERROR : %s", modbus_strerror(errno));
        }
    }
}
...
int modbus_flush(modbus_t *ctx)
{
    ...
    //    if (rc != -1 && ctx->debug) {
    //        log_dbg("[mb]%d bytes flushed\n", rc);
    //    }
    //    return rc;
    // tcflush return 0:success -1:fail
...
static int send_msg(modbus_t *ctx, uint8_t *msg, int msg_length)
{
    ...
    if (ctx->debug)
    {
        ...
        for (i = 0; i < msg_length; i++)
        {
            sprintf(tmpbuf, "%02X ", msg[i]);
            strcat(buf, tmpbuf);
        }
        log_dbg("[mb]%08X,%ld.%ld,tx:%s", (unsigned int)ctx, tv2.tv_sec, tv2.tv_usec, buf);
    }
                                                  ...
                                                  msg_type_t msg_type)
{
    ...
    // log_dbg("%s,function:%d,msg_type:%d",__func__,function,msg_type);
    return length;
}
...
static int receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    if (ctx->debug)
    {
        if (msg_type == MSG_INDICATION)
        {
            ...
            log_dbg("[mb]%08X,Waiting for a indication...", (unsigned int)ctx);
        }
        else
        {
            // printf("Waiting for a confirmation...\n");
            log_dbg("[mb]%08X,Waiting for a confirmation...", (unsigned int)ctx);
        }
    }
    usleep(30000);
    ...
    while (length_to_read != 0)
    {
        ...
        if (ctx->debug)
        {
            ...
            for (i = 0; i < rc; i++)
            {
                sprintf(tmpbuf, "%02X ", msg[msg_length + i]);
                strcat(buf, tmpbuf);
            }
            log_dbg("[mb]%08X,%ld.%ld,rx:%s", (unsigned int)ctx, tv2.tv_sec, tv2.tv_usec, buf);
        }
...
{
    // printf("modbus_set_debug ctx:%08X boolean:%d\n",ctx,boolean);//dbg wanggao
    ctx->debug = boolean;
    // printf("ctx->debug:%d\n",ctx->debug);//dbg wanggao
}
